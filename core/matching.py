...
class LMatchNetwork:
    ...
    def design_multiband(self, freqs: list, Z_loads: list = None) -> list:
        ...
        Parámetros
        ----------
        freqs   : lista de frecuencias [Hz]
        Z_loads : lista de impedancias de carga [Ω] (None → usa self.Z_load)

        Retorna
        -------
        Lista de LMatchResult, uno por frecuencia.
        """
        if Z_loads is None:
            Z_loads = [self.Z_load] * len(freqs)
        return [self.design(f, Zl) for f, Zl in zip(freqs, Z_loads)]
